...
export function validISBN10(isbn: string): boolean {
  if (/[^X0-9]/g.test(isbn) || isbn.length != 10) { return false; }
  if (isbn.indexOf("X") != 9 && isbn.indexOf("X") != -1) { return false; };

  return isbn.split("").reduce((p: number, c: string, i: number): number => {
    if (c === "X") { return p + (10 * (i + 1)); }
    return p + (parseInt(c) * (i + 1));
  }, 0)
    % 11 === 0
    ? true
    : false;
