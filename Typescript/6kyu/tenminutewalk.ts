...
export function isValidWalk(walk: string[]): Boolean {
  let counts: Directions = { n: 0, s: 0, e: 0, w: 0 }
  walk.forEach(function (e) { counts[e as keyof Directions]++; });
  return counts["n"] == counts["s"] && counts["e"] == counts["w"] && walk.length == 10 ? true : false
}
